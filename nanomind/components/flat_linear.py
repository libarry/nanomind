...
class FakeQuantizedLinear(nn.Module):
    ...
    def __init__(self, config, linear: nn.Linear):
        ...
        self.model_quant_type = self._get_quant_type(config)

    def _get_quant_type(self, config):
        if config.w_bits == 8 and config.a_bits == 8 and config.a_per_tensor:
            return "W8A8"
        elif config.w_bits == 8 and config.a_bits == 8 and not config.a_per_tensor:
            return "W8A8_DYNAMIC"
        elif config.w_bits == 4 and config.a_bits == 4:
            return "W4A4_FLATQUANT_DYNAMIC"
        else:
            raise NotImplementedError(f"Unsupported quantization type: {config.w_bits}w{config.a_bits}a")
