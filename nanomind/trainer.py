...
from .processors.flat_quant import (
...
class FlatQuantTrainer:
    ...
    def save_model(self, 
        ...
        Args:
            ...
            model_name: 模型名称
        """
        self.logger.info(f"开始使用新接口保存模型到: {output_dir}")
        save_model(
        ...
        self.logger.info(f"模型保存完成: {output_dir}")
...
def flat_quant_train_and_save(model, calib_data, layer_map, args, logger, 
                             save_dir: Optional[str] = None,
                             model_name: str = "flat_quantized_model"):
    """
    训练并保存模型的便利函数
    ...
    Returns:
        FlatQuantTrainer: 训练器实例
    """
    # 执行训练
    trainer = flat_quant_train(model, calib_data, layer_map, args, logger)
